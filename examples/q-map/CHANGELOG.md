...
### Performance
- **deck.gl rendering: `useDevicePixels: false` + `pickingRadius: 1`:** applied via `deckGlProps` on `<KeplerGl>` in `main.tsx`. On Retina/HiDPI displays this renders 4× fewer pixels (roughly doubling frame rate) with negligible quality loss at map scale. Tighter picking radius reduces hover-picking work on dense scenes.
- **GPKG/ZIP upload: `ST_Simplify` in DuckDB query pipeline:** geometry simplification (Douglas-Peucker, tolerance 0.0001° ≈ 11m) applied in `file-upload-with-url.tsx` before `ST_AsGeoJSON`. Dramatically reduces vertex count for complex polygons (e.g. 5000 → 50 vertices per polygon) making deck.gl rendering smooth on large uploads. Configurable via `VITE_QMAP_UPLOAD_SIMPLIFY_TOLERANCE` (set 0 to disable).
- **GPKG/ZIP upload: eliminate JSON round-trip:** spatial archive uploads now dispatch `addDataToMap` directly with the in-memory FeatureCollection via `processGeojson`, skipping `JSON.stringify → File → Kepler JSON.parse`. Saves ~10-15 seconds on 110MB files.
- **GPKG/ZIP upload: yield-to-main between processing phases:** `yieldToMain()` calls between DuckDB conversion, processGeojson, and addDataToMap let the browser repaint (spinner, progress). Processing phase text shown in UI.
...
### Refactored
- **Standardized dual-dataset tool parameters to `sourceDatasetName` + `targetDatasetName`:** 11 spatial/analytical tools now use consistent parameter naming instead of ad-hoc names (`clipDatasetName`, `leftDatasetName/rightDatasetName`, `adminDatasetName/valueDatasetName`, `datasetAName/datasetBName`, `maskDatasetName`, `tessellationDatasetName`, `stationDatasetName/boundaryDatasetName`, `baselineDatasetName/currentDatasetName`). Also standardized geometry field params to `sourceGeometryField` + `targetGeometryField`. Reduces LLM parameter hallucination (production session `ff4b6d19`: Gemini invented `maskGeometryDatasetName` for `clipDatasetName`). Tool argument normalization (`tool-args-normalization.ts`) maps legacy aliases to canonical names. Updated: tool builders, contracts, backend guardrails, system prompt, eval cases, tests.

### Fixed
- **q-cumber query tools added to `_DATASET_CREATE_OR_UPDATE_TOOLS`:** `queryQCumberTerritorialUnits`, `queryQCumberDatasetSpatial`, and `queryQCumberDataset` with `loadToMap=true` create datasets in the map but were not tracked by the post-create validation chain (wait+count). The guardrail relied on accidental error-recovery (`dataset_not_found_materialization_recovery`) to force wait, and never forced `countQMapRows`. Production session `ff4b6d19` turn 5-11: query loaded dataset, styling attempted before materialization complete, watchdog finalized before count. Now these tools trigger the systematic wait→count→style validation chain.
- **Workflow-boundary scoping for hard cap and watchdog:** hard cap and watchdog now count tool results since the last **workflow boundary** (assistant text-only message without tool_calls) instead of since the last user message. In multi-turn sessions, completed workflow steps no longer exhaust the cap for subsequent queries. Production session `ff4b6d19` turn 24: hard cap fired at count=20 including tool results from a previous completed workflow step; with boundary scoping, only 4 tool calls counted.
- **Hard cap `_TOOL_CALL_WORKFLOW_HARD_CAP` raised from 20 to 25:** multi-turn sessions accumulate tool results across repeated user messages. The old cap of 20 caused premature finalization on the second query in a session when results from the first query were still in the context. Production session `ff4b6d19` turn 24: hard cap fired at count=20 before the spatial query for Natura 2000 could execute.
- **Watchdog `_TOOL_ONLY_NO_TEXT_WATCHDOG_MIN_CALLS` raised from 8 to 12:** multi-step territorial workflows (discovery + query + bbox + spatial_query + wait + count + style + fit) regularly reach 10+ tool calls before producing text. The old threshold of 8 caused premature finalization that bypassed pending validation steps.
- **Empty completion prevention: remove `tools` key entirely on finalization:** when `force_finalize_without_tools` fires (hard cap, watchdog, clarification, zero-match), the upstream payload now omits `tools` and `tool_choice` keys entirely instead of sending `tools=[]` + `tool_choice="none"`. Gemini Flash via OpenRouter produced 0 tokens with the old approach, causing silent `empty_completion` outcomes (observed in production sessions `91d87b0e` turn 3/10 and `cc950272` turn 9). The `tool_choice_none_guard` in `main.py` is also extended to strip tool_calls when `tools` is absent from the payload.
- **Post-mutation validation immune to hard cap override:** when `waitForQMapDataset` or `countQMapRows` are forced by the post-create validation rule, the tool-call hard cap and watchdog can no longer override them with `force_finalize_without_tools`. Production session `91d87b0e` turn 9-10: `clipQMapDatasetByGeometry` created a dataset but the hard cap (20 tool calls) forced finalization before `waitForQMapDataset` could run, leaving workflow score at 60.
- **Frontend batch cap respects per-tool overrides for discovery tools:** `TOOL_CALL_RESPONSE_BATCH_CAP` (default 3) now checks `TOOL_CALL_RESPONSE_BATCH_CAP_OVERRIDES` when all calls in a single LLM response share the same tool name. This allows `getQCumberDatasetHelp` (override 8) and `listQCumberDatasets` (override 6) to execute full batches when the backend forces them. Production session `91d87b0e` turn 2: 4th `getQCumberDatasetHelp` for `natura2000-siti` was dropped by the batch cap, leaving a persistent `contractResponseMismatch=1` through the entire session.
- **Guard: strip tool_calls when `tool_choice: "none"` is violated by upstream provider:** some providers (Gemini Flash via OpenRouter) ignore `tool_choice: "none"` and return tool calls anyway. q-assistant now strips these from both stream SSE chunks and non-stream response bodies before forwarding to the frontend, preventing phantom tool execution and duplicate query turns. Discovered via production chat-audit session `6fe878e2` (2026-04-01): `queryQCumberDatasetSpatial` was issued twice because the model ignored the clarification-mode `tool_choice: "none"` constraint.
- **Runtime guardrail `provider_listing_not_required` no longer allows skipping dataset catalog resolution:** the guidance text now explicitly mandates `listQCumberDatasets(providerId=...)` before any `queryQCumber*` call, preventing the model from hallucinating dataset IDs when provider listing is pruned. Production audit showed 69% tool failure rate (S1 Mar 23) caused by invented dataset IDs on the first turn.
- **`fitQMapToDataset` no longer removed from tool registry mid-session:** the `fit_requires_explicit_map_focus` rule now steers via guidance text only, keeping the tool available. Removing tools mid-session created inconsistency when the model had seen the tool in prior turns and called it from cached context (observed in S4 Mar 25: model called `fitQMapToDataset` after it was removed, ending in `empty_completion`).
- **Compaction of repeated failed tool results:** new `_compact_repeated_failed_tool_results` pass in the chat payload pipeline drops intermediate duplicate failed tool results (keeps first + last per tool), preventing context bloat from retry cascades. S1 accumulated 13 tool results (9 failed) by turn 5; compaction reduces this to max 4 per tool.
- **`circleBufferFromPoint` radius sanity check:** buffers with effective radius < 10m now return a failure hint suggesting the user may have confused meters and kilometers (e.g. "1.5m" vs "1.5km"). Production audit (S3 Mar 23) showed literal 1.5m interpretation of user intent "1.5km".
- **ai-eval variance thresholds raised for `avgExtraToolCalls`:** global span 0.08→0.18, latest 0.1→0.18; area-level span 0.12→0.2, latest 0.15→0.2. Expected effect of mandatory `listQCumberDatasets` discovery step adding one extra tool call to most q-cumber workflows.
- **ai-eval `superlative_admin_unit_show_on_map` cases relaxed:** `fitQMapToDataset` moved from `required_tools_all` to `expected_tools_any`; `listQCumberDatasets` added to `expected_tools_any` for all q-cumber query cases.
- **`tassellateDatasetLayer` tool description clarifies population enrichment requirement:** description now explicitly states that tessellation output contains only `h3_id`/`h3_resolution` and that `populateTassellationFromAdminUnits` must be called to enrich cells with population or admin metrics before coloring. Fixes non-deterministic skip of enrichment step (was ~33% fail rate on Brescia tessellation+population case).
- **Runtime guardrail `tessellation_requires_population_enrichment`:** when objective mentions population and `tassellateDatasetLayer` succeeded without a subsequent `populateTassellationFromAdminUnits`, guidance is injected to force the enrichment step.
- **ai-eval `arch_spatial_autocorrelation_lisa` stabilized:** added `computeQMapHotspotAnalysis` to `expected_tools_any`; removed `HH`/`LL` from `expected_keywords_any` (too granular — model includes them in visualization but not always in text). 5/5 pass after fix (was ~60% before).
...
### Added
- **Geometry drawing tools (`centroidOfDataset`, `circleBufferFromPoint`, `drawGeometryToDataset`):** three new AI assistant tools for programmatic geometry creation. `centroidOfDataset` computes the geographic center of a dataset. `circleBufferFromPoint` creates a circular buffer polygon from a point + radius. `drawGeometryToDataset` creates arbitrary geometries (point, line, polygon, circle) with computed metrics (area_m2, area_km2, length_m, perimeter_m, radius_km). All produce reusable datasets for spatial queries.
